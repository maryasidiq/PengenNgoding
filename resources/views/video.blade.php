    ...
    <section class="mb-14">
      <h1 class="text-lg md:text-3xl font-bold text-gray-[#124C76] mb-1">Mau belajar apa hari ini?</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <!-- Card 1 - Frontend -->
          <a href="{{ route('video.kategori.konten', 'frontend') }}" class="block" data-aos="zoom-out-down">
          ...
          <!-- Card 2 - Android -->
          <a href="{{ route('video.kategori.konten', 'android') }}" class="block" data-aos="zoom-out-down">
          ...
          <!-- Card 3 -->
          <a href="{{ route('video.kategori') }}" class="block" data-aos="zoom-out-down">
          ...
          <div class="rounded-lg border border-gray-200 overflow-hidden shadow hover:shadow-md transition block relative"
            ...
            <a href="{{ route('video.bab', ['id' => encrypt($konten->video->id), 'bab_id' => encrypt($konten->id)]) }}">
              ...
              <div class="p-4 flex flex-col gap-2">
                <h3 class="text-sm font-semibold text-gray-800">
                  {{ $konten->video->judul }} - {{ $konten->judul }}
                </h3>
            ...
            @auth
              <div class="absolute top-2 right-2 flex items-center space-x-1">
                <a href="{{ route('admin.video.konten.edit', ['video' => encrypt($konten->video->id), 'konten' => encrypt($konten->id)]) }}"
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24"
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 
                                      002-2v-5m-1.414-9.414 a2 2 0 112.828 2.828L11.828 
                                      15H9v-2.828 l8.586-8.586z" />
                  </svg>
                  <span>Edit</span>
                </a>
              </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-8">
        @foreach ($videos as $video)
          <article class="flex flex-col items-center text-center hover:scale-110 transition" data-aos="zoom-out-down">
            <a href="{{ route('video.detail', ['id' => encrypt($video->id)]) }}"
