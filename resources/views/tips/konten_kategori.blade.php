...
<body class="bg-white text-gray-700 font-sans selection:bg-blue-600 selection:text-white">
  ...
  <main class="px-6 md:px-8 container max-w-7xl mx-auto pt-24 md:pt-28 pb-12">
    <section class="max-w-7xl mx-auto">
      <header class="mb-6">
        <p class="text-sm font-semibold text-blue-700 mb-2">Tips</p>
        <h2 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
          Kategori: {{ $kategori }}
        </h2>
        <p class="text-gray-600 text-sm md:text-base mt-1 max-w-xl">
          Ditemukan {{ $konten->total() }} tips dalam kategori <span class="font-semibold">{{ $kategori }}</span>.
        </p>
        <a href="{{ route('tips.kategori') }}" class="inline-block mt-4 text-blue-600 hover:underline text-sm">
      ...
      <a href="{{ route('tips.bab', ['id' => $item->tips_id, 'bab_id' => $item->id]) }}"
        ...
        <img src="{{ asset($item->gambar) }}" class="w-full h-48 object-cover" alt="{{ $item->tips->judul }}">
        <div class="p-4 flex flex-col gap-2">
        <h3 class="text-sm font-semibold text-gray-800">
          {{ $item->tips->judul }} - {{ $item->judul }}
        </h3>
