...
<body class="bg-white text-gray-700 font-sans selection:bg-blue-600 selection:text-white">
  ...
  <main class="px-6 md:px-8 container max-w-7xl mx-auto pt-24 md:pt-28 pb-12">
    <section class="max-w-7xl mx-auto">
      <header class="mb-6">
        <p class="text-sm font-semibold text-blue-700 mb-2">Artikel</p>
        <h2 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
          Kategori: {{ $kategori }}
        </h2>
        <p class="text-gray-600 text-sm md:text-base mt-1 max-w-xl">
          Ditemukan {{ $konten->total() }} artikel dalam kategori <span class="font-semibold">{{ $kategori }}</span>.
        </p>
        <a href="{{ route('artikel.kategori') }}" class="inline-block mt-4 text-blue-600 hover:underline text-sm">
          ...
          <div class="rounded-lg border border-gray-200 overflow-hidden shadow hover:shadow-md transition block relative">
            ...
            @auth
              <div class="absolute top-2 right-2 flex items-center space-x-1">
                <a href="{{ route('admin.artikel.konten.edit', ['artikel' => encrypt($item->artikel->id), 'konten' => encrypt($item->id)]) }}"
